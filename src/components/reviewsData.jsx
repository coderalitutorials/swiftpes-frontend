...
const highImpactReviews = [
  {
    ...
    // role: "Property Resident",
    image: "/images/reviews/user1.webp",
    text: "Absolutely fantastic response from SwiftPes. Had a severe mouse infestation in our structural loft space. The team arrived promptly, tracked down entry pathways, and locked the area down permanently.",
    rating: "5.0 Verified",
  },
  {
    id: 2,
  ...
  //   role: "Estate Landlord",
  //   image: "/images/reviews/user6.webp",
  //   text: "Had a highly persistent bed bug outbreak across two flat layouts. SwiftPes resolved it completely using specialized deep thermal heat treatments. Unmatched reliability and execution.",
  //   rating: "5.0 Verified",
  // },
  {
    id: 4,
    ...
    // role: "Commercial Manager",
    image: "/images/reviews/user3.webp",
    text: "Outstanding commercial contract execution. SwiftPes setup non-toxic bio-barriers around our entire logistics warehouse layout. Constant updates, total peace of mind.",
    rating: "5.0 Verified",
  },
];
...
export default function PesSaveTestimonials() {
  return (
    <section className="relative py-28 md:py-36 bg-[#120c1a] overflow-hidden z-20 border-b border-white/[0.04]">
      {/* Structural Backdrop Grid & Soft Brand Lights */}
      <div className="absolute inset-0 bg-[radial-gradient(#ffffff01_1px,transparent_1px)] [background-size:2rem_2rem] pointer-events-none opacity-40" />
      <div className="absolute -top-40 left-1/4 w-[360px] h-[360px] bg-[#4B1B67]/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -bottom-40 right-1/4 w-[360px] h-[360px] bg-[#A35C37]/5 rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-6 md:px-12 mb-20 relative z-10 animate-[fadeUp_0.7s_ease-out_both]">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4 max-w-xl text-left">
            <div className="inline-flex items-center gap-2 bg-white/[0.02] border border-white/10 px-3.5 py-1 rounded-md shadow-lg backdrop-blur-sm">
              <MessageSquare className="w-3 h-3 text-[#A35C37]" />
            ...
            <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-white uppercase leading-[1.1]">
              RELIABILITY <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4B1B67] to-[#A35C37]">
                WITHOUT COMPROMISE
              </span>
            </h2>
          </div>
          ...
          <div className="max-w-xs text-left md:text-right">
            <p className="text-slate-400 text-xs sm:text-sm leading-relaxed font-normal">
              Real-time response documentation collected across verified 
              residential and commercial deployments in the Worcester zones.
            </p>
          </div>
        </div>
      </div>

      {/* Infinite Horizontal Review Marquee */}
      <div className="relative w-full flex flex-col gap-6 select-none overflow-hidden max-w-[100vw]">
        <div className="w-full overflow-hidden relative mask-gradient">
          <div className="flex gap-6 w-max px-4 animate-[reviewMarquee_32s_linear_infinite] hover:[animation-play-state:paused] will-change-transform">
            {[...highImpactReviews, ...highImpactReviews].map((review, idx) => (
              <ReviewCard key={`${review.id}-${idx}`} review={review} />
            ))}
          </div>
...
function ReviewCard({ review }) {
  return (
    <div className="w-[360px] sm:w-[420px] bg-white/[0.015] border border-white/[0.05] p-6 sm:p-8 rounded-2xl flex flex-col justify-between shadow-2xl relative overflow-hidden shrink-0 group transition-all duration-300 hover:border-[#A35C37]/30 hover:bg-white/[0.025]">
      <div>
        <div className="flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className="w-3 h-3 fill-[#A35C37] text-[#A35C37]"
                strokeWidth={0}
              />
            ))}
          </div>

          <span className="text-[9px] font-mono font-bold tracking-wider text-slate-300 bg-white/[0.03] border border-white/10 px-2.5 py-0.5 rounded shadow-sm">
            {review.rating}
          </span>
        </div>
        ...
        <div className="relative text-left">
          <Quote
            className="absolute top-0 left-0 w-8 h-8 text-[#A35C37]/5 -translate-x-3 -translate-y-3 pointer-events-none"
            strokeWidth={1}
          />
          <p className="text-slate-300 text-xs sm:text-sm leading-relaxed font-normal relative z-10 pl-1">
            "{review.text}"
      ...
      <div className="flex items-center gap-3.5 pt-4 mt-6 border-t border-white/[0.04]">
        <div className="w-9 h-9 rounded-xl overflow-hidden bg-white/[0.02] border border-white/10 p-0.5 shrink-0">
          <img
            src={review.image}
            alt={review.name}
            className="w-full h-full object-cover rounded-lg grayscale opacity-75 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300"
            loading="lazy"
            decoding="async"
          />
        </div>
        ...
        <div className="text-left min-w-0 flex-grow">
          <div className="flex items-center gap-1">
            ...
            <ShieldCheck className="w-3.5 h-3.5 text-[#A35C37] shrink-0" />
          </div>
