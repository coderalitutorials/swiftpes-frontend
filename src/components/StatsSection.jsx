...
export default function PesSaveStatsSection() {
  ...
  const stats = [
    {
      number: "15+",
      label: "Years Service Depth",
      icon: Clock,
      desc: "Years of reliable emergency experience across the local region.",
    },
    {
      number: "5/5",
      label: "Client Trust Rating",
      icon: Shield,
      desc: "Top-tier rating based on prompt response and eradication results.",
    },
    {
      number: "100%",
      label: "Eradication Guarantee",
      icon: Target,
      desc: "Full service protection warranty ensuring completely secure homes.",
    },
    {
      number: "2K+",
      label: "Properties Protected",
      icon: Zap,
      desc: "Successful active treatment setups completed in town grids.",
    },
  ];

  return (
    <section className="bg-[#120c1a] py-20 md:py-28 text-white relative overflow-hidden z-30 border-y border-white/[0.04]">
      {/* Structural Minimal Background Lines */}
      <div className="absolute top-0 left-1/4 w-[1px] h-full bg-gradient-to-b from-white/0 via-white/[0.02] to-white/0 pointer-events-none" />
      <div className="absolute top-0 left-2/3 w-[1px] h-full bg-gradient-to-b from-white/0 via-white/[0.02] to-white/0 pointer-events-none" />
      <div className="absolute top-1/3 left-0 w-full h-[1px] bg-gradient-to-r from-white/0 via-white/[0.02] to-white/0 pointer-events-none" />

      {/* Brand Ambient Glows */}
      <div className="absolute -top-40 right-[10%] w-[360px] h-[360px] bg-[#4B1B67]/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -bottom-20 left-[-5%] w-[320px] h-[320px] bg-[#A35C37]/10 rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          
          {/* Brand Promo Column */}
          <div className="lg:col-span-5 bg-gradient-to-br from-[#1b1224]/50 to-[#120c1a] border border-white/[0.08] rounded-3xl p-8 md:p-12 flex flex-col justify-between text-left shadow-2xl relative overflow-hidden group animate-[fadeUp_0.7s_ease-out_both]">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#4B1B67]/10 rounded-full blur-2xl pointer-events-none group-hover:bg-[#4B1B67]/20 transition-colors duration-300" />

            <div className="space-y-4">
              <div className="inline-flex items-center gap-2.5 bg-white/[0.03] border border-white/10 px-3 py-1 rounded-full backdrop-blur-sm">
                <span className="w-1.5 h-1.5 rounded-full bg-[#A35C37] opacity-90" />
                <span className="text-[10px] font-mono tracking-[0.2em] text-[#A35C37] uppercase font-bold">
                  Track Record Live
                </span>
              </div>
              ...
              <p className="text-sm text-slate-400 leading-relaxed max-w-sm pt-2">
                Real-world performance parameters monitoring our service efficiency and premium pest eradication rates across Worcester.
              </p>
            </div>
            ...
            <div className="pt-12 lg:pt-0">
              <button
                onClick={() => navigate("/contact-us")}
                className="w-full sm:w-auto inline-flex items-center justify-between gap-6 bg-gradient-to-r from-[#4B1B67] to-[#A35C37] text-white rounded-2xl px-6 py-4 text-xs font-black tracking-widest uppercase transition-transform duration-200 shadow-lg shadow-[#4B1B67]/20 hover:-translate-y-0.5 active:scale-[0.98] group/btn cursor-pointer"
              >
                <span>Book Service Online</span>
                <div className="w-7 h-7 rounded-xl bg-white/10 flex items-center justify-center transition-transform duration-200 group-hover/btn:translate-x-1">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {stats.map((stat, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-gradient-to-b from-[#1b1224]/30 to-transparent border border-white/[0.05] rounded-3xl p-6 md:p-8 text-left flex flex-col justify-between transition-transform duration-200 relative group overflow-hidden shadow-xl hover:-translate-y-1 animate-[fadeUp_0.7s_ease-out_both]"
                  style={{ animationDelay: `${idx * 0.08}s` }}
                >
                  <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-white/0 to-transparent group-hover:via-[#A35C37]/30 transition-colors duration-300" />

                  <div>
                    <div className="flex items-center justify-between gap-4 mb-6">
                      <div className="w-9 h-9 rounded-xl bg-white/[0.02] border border-white/10 flex items-center justify-center transition-colors duration-200 group-hover:border-[#4B1B67]/30 group-hover:bg-[#4B1B67]/10">
                        <Icon
                          className="w-4 h-4 text-slate-400 group-hover:text-[#A35C37] transition-colors duration-200"
                          strokeWidth={1.8}
                        />
                      </div>

                      <span className="font-mono text-[10px] font-bold text-white/5 group-hover:text-[#4B1B67]/30 transition-colors duration-200">
                        // SCORE_0{idx + 1}
                      </span>
                    </div>
                    ...
                    <h4 className="text-xs font-bold tracking-widest text-[#A35C37] uppercase mt-2">
                      {stat.label}
                    </h4>
                  </div>

                  <p className="text-xs text-slate-400 mt-4 leading-relaxed group-hover:text-slate-300 transition-colors duration-200">
                    {stat.desc}
                  </p>

                  <div className="w-full h-[1.5px] bg-white/5 mt-6 relative overflow-hidden rounded-full">
                    <div className="absolute top-0 left-0 h-full w-12 bg-gradient-to-r from-[#4B1B67] to-[#A35C37] transition-transform duration-500 -translate-x-full group-hover:translate-x-[400%]" />
                  </div>
                </div>
              );
            })}
