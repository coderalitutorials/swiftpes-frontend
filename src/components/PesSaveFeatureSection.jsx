...
export default function PesSaveFeatureSection() {
  ...
  return (
    <section className="relative py-28 md:py-36 bg-[#120c1a] overflow-hidden z-20 border-b border-white/[0.04] group/section">
      {/* Background Image Layer with adjusted lower opacity masks for sharp clarity */}
      <div className="absolute inset-0 z-0 select-none pointer-events-none">
        <img
          src="/images/moth-control.webp"
          alt="Professional residential pest control treatment context background"
          className="w-full h-full object-cover object-center opacity-65 contrast-[1.05] brightness-95 saturate-[1.05]"
          loading="lazy"
          decoding="async"
        />
        {/* Softened radial and linear gradients so the visual details stay clear */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#120c1a]/75 via-[#120c1a]/25 to-[#120c1a]/75" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_20%,#120c1a_75%)]" />
      </div>

      {/* Structured Minimal Grid Overlay */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff02_1px,transparent_1px),linear-gradient(to_bottom,#ffffff02_1px,transparent_1px)] bg-[size:4rem_4rem] pointer-events-none opacity-30" />

      {/* Brand Purple Glow Center Accent */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] h-[280px] bg-[#4B1B67]/20 rounded-full blur-3xl pointer-events-none opacity-40 group-hover/section:opacity-60 transition-opacity duration-300" />
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 relative z-10 text-center">
        <div className="space-y-12 animate-[fadeUp_0.7s_ease-out_both]">
          ...
          <div className="space-y-6 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 bg-[#120c1a]/90 border border-white/10 px-3.5 py-1 rounded-md shadow-lg backdrop-blur-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-[#A35C37] opacity-90" />
              <span className="text-[10px] font-mono tracking-[0.25em] text-[#A35C37] uppercase font-black">
                Service Blueprint
              </span>
            </div>

            <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-white uppercase leading-[1.1] [text-shadow:0_4px_16px_rgba(18,12,26,0.95)]">
              Complete Eradication <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-slate-400">
                When You Secure Expert Help
              </span>
            </h2>

            <p className="text-slate-200 text-sm sm:text-base leading-relaxed max-w-2xl mx-auto font-medium [text-shadow:0_3px_14px_rgba(18,12,26,0.95)]">
              Our specialized Worcester technicians deploy targeted eco-certified 
              systems to neutralize pests permanently. We focus on long-term preventive 
              restriction protocols so your property remains completely secure, safe, and clean.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl mx-auto pt-2">
            ...
            {/* Card 1 */}
            <div className="bg-[#120c1a]/90 hover:bg-[#120c1a]/85 border border-white/[0.08] hover:border-[#4B1B67]/60 p-6 rounded-2xl transition-all duration-300 group/card flex flex-col items-center text-center shadow-2xl backdrop-blur-sm">
              <div className="w-10 h-10 rounded-xl bg-white/[0.02] border border-white/10 flex items-center justify-center group-hover/card:bg-[#4B1B67]/20 group-hover/card:border-[#4B1B67] transition-colors duration-200">
                <ShieldCheck className="w-5 h-5 text-[#A35C37]" />
              </div>
              <h4 className="text-white font-bold text-xs uppercase tracking-wider mt-4 mb-1">
                Certified Team
              </h4>
              <p className="text-slate-400 text-[11px] leading-normal font-normal max-w-[200px] group-hover:text-slate-300 transition-colors duration-200">
                Client-oriented, fully trained and certified local pest experts.
              </p>
            </div>

            {/* Card 2 */}
            <div className="bg-[#120c1a]/90 hover:bg-[#120c1a]/85 border border-white/[0.08] hover:border-[#A35C37]/60 p-6 rounded-2xl transition-all duration-300 group/card flex flex-col items-center text-center shadow-2xl backdrop-blur-sm">
              <div className="w-10 h-10 rounded-xl bg-white/[0.02] border border-white/10 flex items-center justify-center group-hover/card:bg-[#A35C37]/20 group-hover/card:border-[#A35C37] transition-colors duration-200">
                <Leaf className="w-5 h-5 text-[#A35C37]" />
              </div>
              <h4 className="text-white font-bold text-xs uppercase tracking-wider mt-4 mb-1">
                Eco Formulations
              </h4>
              <p className="text-slate-400 text-[11px] leading-normal font-normal max-w-[200px] group-hover:text-slate-300 transition-colors duration-200">
                Premium, target-specific and safe treatment compounds.
              </p>
            </div>
            ...
            <div className="bg-[#120c1a]/90 hover:bg-[#120c1a]/85 border border-white/[0.08] hover:border-[#4B1B67]/60 p-6 rounded-2xl transition-all duration-300 group/card flex flex-col items-center text-center shadow-2xl backdrop-blur-sm">
              <div className="w-10 h-10 rounded-xl bg-white/[0.02] border border-white/10 flex items-center justify-center group-hover/card:bg-[#4B1B67]/20 group-hover/card:border-[#4B1B67] transition-colors duration-200">
                <Zap className="w-5 h-5 text-[#A35C37]" />
              </div>
              <h4 className="text-white font-bold text-xs uppercase tracking-wider mt-4 mb-1">
                Total Safety
              </h4>
              <p className="text-slate-400 text-[11px] leading-normal font-normal max-w-[200px] group-hover:text-slate-300 transition-colors duration-200">
                Complete long-term property protection and peace of mind.
              </p>
            </div>
          </div>
          ...
          <div className="pt-2 flex justify-center">
            <button
              onClick={() => navigate("/contact-us")}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-4 bg-[#120c1a]/90 hover:bg-[#120c1a]/95 border border-white/10 hover:border-[#A35C37]/60 text-white font-black text-xs tracking-widest uppercase px-10 py-4 rounded-xl transition-all duration-200 group shadow-2xl cursor-pointer overflow-hidden relative hover:-translate-y-0.5 active:scale-[0.98] backdrop-blur-sm"
            >
              <div className="flex items-center gap-2">
              ...
              <div className="w-5 h-5 rounded-lg bg-white/5 flex items-center justify-center group-hover:bg-[#A35C37] transition-colors duration-200">
                <ArrowRight
                  className="w-3 h-3 text-white transition-transform duration-200 group-hover:translate-x-0.5"
                  strokeWidth={3}
                />
